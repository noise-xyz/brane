...
Brane exposes `eth_simulateV1` via `PublicClient.simulateCalls()`. This lets you **dry-run** one or more calls (in sequence), optionally applying **state overrides** (balance/nonce/code/storage) and collecting richer output such as **per-call gas used**, **logs**, and **asset changes**.
...
try {
    var result = publicClient.simulateCalls(request);
...
- **Spec reference**: See the upstream API discussion in [execution-apis PR #484](https://github.com/ethereum/execution-apis/pull/484).
