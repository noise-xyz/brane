...
flowchart TD
    ...
    subgraph BRANE["Brane (sealed interface)"]
        direction LR
        READER["Brane.Reader<br/><i>read-only ops</i>"] ~~~ SIGNER["Brane.Signer<br/><i>read + write ops</i>"]
    end

    subgraph IMPLS["Implementations"]
        direction LR
        DEFAULT_READER["DefaultReader"] ~~~ DEFAULT_SIGNER["DefaultSigner"]
    end

    subgraph PROVIDER["BraneProvider (interface)"]
        direction LR
        HTTP["HttpBraneProvider<br/>• HttpClient<br/>• Stateless"] ~~~ WS["WebSocketProvider<br/>• Netty<br/>• Disruptor<br/>• Subscriptions"]
    end
