...
flowchart TD
    ...
    subgraph BRANE["Brane (sealed interface)"]
        direction LR
        READER["Brane.Reader<br/><i>read-only operations</i>"] ~~~ SIGNER["Brane.Signer<br/><i>read + write operations</i>"] ~~~ TESTER["Brane.Tester<br/><i>test node operations</i>"]
    end

    subgraph IMPLS["Implementations"]
        direction LR
        DEFAULT_READER["DefaultReader<br/><i>implements Reader</i>"] ~~~ DEFAULT_SIGNER["DefaultSigner<br/><i>implements Signer</i>"] ~~~ DEFAULT_TESTER["DefaultTester<br/><i>implements Tester</i>"]
    end
    ...
    DEFAULT_TESTER -.->|delegates to| DEFAULT_SIGNER
    IMPLS --> PROVIDER
    PROVIDER --> NODE
```
