...
```java
WethContract weth = BraneContract.bind(address, abiJson, publicClient, walletClient, WethContract.class);
...
**Workaround:** Use `ReadOnlyContract` or `ReadWriteContract` and manually decode the response:

```java
ReadOnlyContract contract = ReadOnlyContract.from(address, abi, publicClient);
// Call returns raw hex, decode manually
String rawHex = contract.call("getPosition", String.class);
// Decode using AbiDecoder...
```
