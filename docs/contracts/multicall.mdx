...
MulticallBatch batch = MulticallBatch.create(publicClient);
...
```java
import io.brane.contract.MulticallBatch;
...
```java
import io.brane.contract.MulticallBatch;
import io.brane.contract.BatchHandle;
import io.brane.contract.BatchResult;
...
public class MulticallExample {
    public static void main(String[] args) {
        PublicClient publicClient = // ... your client
...
BatchHandle<Integer> tokenCount = batch.add(nft.balanceOf(user));
...
MulticallBatch batch2 = MulticallBatch.create(publicClient);
// ... add new calls ...
```
