...
Brane's WebSocket transport enables persistent connections for real-time subscriptions and lower-latency RPC calls.
...
```java
import io.brane.rpc.Brane;
import io.brane.rpc.WebSocketConfig;
import io.brane.rpc.WebSocketConfig.WaitStrategyType;
import io.brane.rpc.WebSocketProvider;
...
<Tip>
For **type-safe subscriptions**, use [`client.onNewHeads()`](/public-client/subscriptions) and [`client.onLogs()`](/public-client/subscriptions) instead of the raw provider API shown below.
</Tip>
...
// Or run directly on the Netty I/O thread (⚠️ use with caution)
...
```java
import io.brane.rpc.WebSocketProvider;
import io.brane.rpc.WebSocketProvider.ConnectionState;
...
var balance = reader.getBalance(Address.from("0x..."));
