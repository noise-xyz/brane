...
var provider = WebSocketProvider.create(config);
```
...
<Tip>
For **type-safe subscriptions**, use [`PublicClient.subscribeToNewHeads()`](/public-client/subscriptions) and [`subscribeToLogs()`](/public-client/subscriptions) instead of the raw provider API shown below.
</Tip>
