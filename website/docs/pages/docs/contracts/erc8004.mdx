...
Bind an Ethereum wallet to an agent identity using EIP-712 signed authorization:

```java
import sh.brane.core.crypto.erc8004.Erc8004Wallet;
import sh.brane.core.crypto.eip712.Eip712Domain;

// Create domain for the identity registry
Eip712Domain domain = Erc8004Wallet.domain(chainId, identityRegistryAddress);

// Sign wallet binding
Signature sig = Erc8004Wallet.signWalletBinding(
        agentId, newWallet, deadline, domain, signer);
```
...
```java
// Simple feedback with score
FeedbackValue score = FeedbackValue.of(95, 2);  // 0.95
agents.giveFeedback(agentId, score, "quality", "fast", "/api/chat", "", null);
...
List<AgentRegistered> registered = agents.getRegisteredEvents(fromBlock, toBlock);

// Feedback events
List<FeedbackSubmitted> feedback = agents.getFeedbackEvents(fromBlock, toBlock);

// Revocation events
List<FeedbackRevoked> revoked = agents.getRevokedEvents(fromBlock, toBlock);
```

Event records use raw ABI types and provide typed converter methods:

```java
AgentRegistered event = registered.get(0);
