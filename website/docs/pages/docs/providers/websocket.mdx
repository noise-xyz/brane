...
:::tip
Set `writeIdleTimeout` shorter than `readIdleTimeout` so pings are sent before the read timeout triggers. The defaults (15s write, 30s read) work well for most environments.
:::
