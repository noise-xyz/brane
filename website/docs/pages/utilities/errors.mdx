...
See [EIP-712 Typed Data](/signer/eip712) for details on typed data signing.
...
See [Blob Transactions](/signer/blobs) for details on EIP-4844 support.
...
Thrown when `eth_simulateV1` is not supported by the RPC node. See [Transaction Simulation](/reader/simulate) for details.
