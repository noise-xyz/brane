...
public class TransactionDecoder {
        ...
        private static RawTransaction decodeEIP7702Transaction(final byte[] transaction) {
                ...
                final List<AuthorizationTuple> authorizationList = decodeAuthorizationList(
                                ((RlpList) fields.get(9)).getValues());
                ...
                if (fields.size() == UNSIGNED_EIP7702TX_RLP_LIST_SIZE) {
                        ...
                        final int yParity = Numeric.toBigInt(((RlpString) fields.get(10)).getBytes()).intValue();
                        final byte[] rBytes = Numeric.toBytesPadded(
                                        Numeric.toBigInt(((RlpString) fields.get(11)).getBytes()), 32);
                        final byte[] sBytes = Numeric.toBytesPadded(
        ...
        private static TransactionType getTransactionType(final byte[] transaction) {
                ...
                else if (firstByte == TransactionType.EIP7702.getRlpType())
        ...
        private static RawTransaction decodeEIP1559Transaction(final byte[] transaction) {
                ...
                List<AccessListObject> accessList = decodeAccessList(((RlpList) values.getValues().get(8)).getValues());
        ...
        private static RawTransaction decodeEIP2930Transaction(final byte[] transaction) {
                ...
                List<AccessListObject> accessList = decodeAccessList(((RlpList) values.getValues().get(7)).getValues());
                ...
                return rlpList.stream()
                                ...
                                .map(
                                                tuple -> {
                                                        ...
                                                        final BigInteger authChainId = ((RlpString) elements.get(0))
                                                                        .asPositiveBigInteger();
                                                        final String address = ((RlpString) elements.get(1)).asString();
                                                        final BigInteger authNonce = ((RlpString) elements.get(2))
                                                                        .asPositiveBigInteger();
                                                        final BigInteger yParity = ((RlpString) elements.get(3))
                                                                        .asPositiveBigInteger();
                                                        final BigInteger rValue = ((RlpString) elements.get(4))
                                                                        .asPositiveBigInteger();
                                                        final BigInteger sValue = ((RlpString) elements.get(5))
                                                                        .asPositiveBigInteger();

                                                        // Convert or store them as desired; for example:
                                                        return new AuthorizationTuple(
                                                                        authChainId, address, authNonce, yParity,
                                                                        rValue, sValue);
                                                })
                                .toList();
        }
