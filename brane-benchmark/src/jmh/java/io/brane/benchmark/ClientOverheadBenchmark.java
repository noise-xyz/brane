...
public class ClientOverheadBenchmark {
    ...
    private static final String CHAIN_ID_RESPONSE = 
        "{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":\"0x1\"}";
    
    private static final String BLOCK_NUMBER_RESPONSE = 
        "{\"jsonrpc\":\"2.0\",\"id\":2,\"result\":\"0x12a05f200\"}";
    
    private static final String BALANCE_RESPONSE = 
    ...
    // ThreadLocal StringBuilder for Brane (simulating what UltraFastWebSocketProvider does)
     ...
     * This extracts 'id' and 'result' or 'error' without creating intermediate objects.
     */
    private ParsedResponse braneParseResponse(String json) {
        ...
        while (i < len) {
            ...
            if (chars[i] == '"') {
                int keyStart = i + 1;
                int keyEnd = keyStart;
                while (keyEnd < len && chars[keyEnd] != '"') keyEnd++;
                
                String key = new String(chars, keyStart, keyEnd - keyStart);
                i = keyEnd + 1;
                ...
                if ("id".equals(key)) {
                    // Parse number
                    int numStart = i;
                    while (i < len && (Character.isDigit(chars[i]) || chars[i] == '-')) i++;
    ...
    private String extractValue(char[] chars, int start, int len) {
        if (chars[start] == '"') {
            ...
            while (end < len && chars[end] != '"') {
                if (chars[end] == '\\') end++; // skip escaped char
                end++;
            }
            ...
            // Primitive
            int end = start;
            while (end < len && chars[end] != ',' && chars[end] != '}' && chars[end] != ']') end++;
    ...
    private int skipValue(char[] chars, int start, int len) {
        ...
        if (c == '"') {
            ...
            // Primitive
            int i = start;
            while (i < len && chars[i] != ',' && chars[i] != '}' && chars[i] != ']') i++;
            return i;
        }
    }
