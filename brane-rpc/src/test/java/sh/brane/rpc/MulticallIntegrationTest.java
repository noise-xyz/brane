// SPDX-License-Identifier: MIT OR Apache-2.0
package sh.brane.rpc;

import static org.junit.jupiter.api.Assertions.*;
import static org.junit.jupiter.api.Assumptions.assumeTrue;

import java.math.BigInteger;

import org.junit.jupiter.api.BeforeAll;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.condition.EnabledIfSystemProperty;

import sh.brane.core.abi.Abi;
import sh.brane.core.builder.TxBuilder;
import sh.brane.core.crypto.PrivateKeySigner;
import sh.brane.core.model.TransactionReceipt;
import sh.brane.core.types.Address;
import sh.brane.core.types.HexData;
import sh.brane.core.types.Wei;

/**
 * Integration test for MulticallBatch against real Multicall3 on Anvil.
 *
 * <p>Requires Anvil running with a mainnet fork (Multicall3 must be deployed):
 * <pre>
 * anvil --fork-url https://eth.llamarpc.com
 * </pre>
 *
 * <p>Run with:
 * <pre>
 * ./gradlew :brane-rpc:test -Dbrane.integration.tests=true --tests "*MulticallIntegrationTest"
 * </pre>
 */
@EnabledIfSystemProperty(named = "brane.integration.tests", matches = "true")
class MulticallIntegrationTest {

    private static final String ERC20_ABI = """
            [{
                "name": "balanceOf",
                "type": "function",
                "stateMutability": "view",
                "inputs": [{"name": "account", "type": "address"}],
                "outputs": [{"name": "", "type": "uint256"}]
            }, {
                "name": "symbol",
                "type": "function",
                "stateMutability": "view",
                "inputs": [],
                "outputs": [{"name": "", "type": "string"}]
            }, {
                "name": "decimals",
                "type": "function",
                "stateMutability": "view",
                "inputs": [],
                "outputs": [{"name": "", "type": "uint8"}]
            }, {
                "name": "name",
                "type": "function",
                "stateMutability": "view",
                "inputs": [],
                "outputs": [{"name": "", "type": "string"}]
            }, {
                "type": "constructor",
                "inputs": [{"name": "initialSupply", "type": "uint256"}]
            }]
            """;

    // BraneToken ERC20 bytecode (name="Brane Token", symbol="BRANE", decimals=18)
    private static final String ERC20_BYTECODE = "";

    private static final Address MULTICALL3_ADDRESS = new Address("0xcA11bde05977b3631167028862bE2a173976CA11");

    private static BraneProvider provider;
    private static Brane.Signer braneClient;
    private static Address deployer;
    private static Address tokenAddress;

    interface Erc20 {
        BigInteger balanceOf(Address account);
        String symbol();
        BigInteger decimals();
        String name();
    }

    @BeforeAll
    static void setup() {
        String rpcUrl = System.getProperty("brane.examples.rpc", "http://127.0.0.1:8545");
        String pk = "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"; // Anvil account #0

        provider = HttpBraneProvider.builder(rpcUrl).build();
        var signer = new PrivateKeySigner(pk);
        braneClient = Brane.connect(rpcUrl, signer);
        deployer = signer.address();

        // Check if Multicall3 is deployed (required for these tests)
        // Use eth_getCode to verify contract exists at the address
        JsonRpcResponse codeResponse = provider.send("eth_getCode",
                java.util.List.of(MULTICALL3_ADDRESS.value(), "latest"));
        String code = codeResponse.result() != null ? codeResponse.result().toString() : "0x";
        boolean multicall3Deployed = code != null && !code.equals("0x") && code.length() > 2;
        assumeTrue(multicall3Deployed,
                "Multicall3 not deployed at " + MULTICALL3_ADDRESS.value() +
                ". Run Anvil with --fork-url to fork a chain with Multicall3.");

        // Deploy a test ERC20 token
        tokenAddress = deployErc20(BigInteger.valueOf(1_000_000));
    }

    @Test
    void batchesMultipleViewCalls() {
        MulticallBatch batch = braneClient.batch();
        Erc20 token = batch.bind(Erc20.class, tokenAddress, ERC20_ABI);

        // Batch multiple calls
        BatchHandle<BigInteger> balanceHandle = batch.add(token.balanceOf(deployer));
        BatchHandle<String> symbolHandle = batch.add(token.symbol());
        BatchHandle<BigInteger> decimalsHandle = batch.add(token.decimals());
        BatchHandle<String> nameHandle = batch.add(token.name());

        batch.execute();

        // Verify all results succeeded
        assertTrue(balanceHandle.result().success(), "balanceOf should succeed");
        assertTrue(symbolHandle.result().success(), "symbol should succeed");
        assertTrue(decimalsHandle.result().success(), "decimals should succeed");
        assertTrue(nameHandle.result().success(), "name should succeed");

        // Verify values match expected on-chain data
        assertEquals(BigInteger.valueOf(1_000_000), balanceHandle.result().data());
        assertEquals("BRANE", symbolHandle.result().data());
        assertEquals(BigInteger.valueOf(18), decimalsHandle.result().data());
        assertEquals("Brane Token", nameHandle.result().data());
    }

    @Test
    void verifiesMulticall3ContractExists() {
        // Verify the Multicall3 contract is deployed at the deterministic address
        // getBlockNumber() is a simple view function on Multicall3
        String multicall3Abi = """
                [{"name": "getBlockNumber", "type": "function", "stateMutability": "view",
                  "inputs": [], "outputs": [{"name": "", "type": "uint256"}]}]
                """;

        interface Multicall3 {
            BigInteger getBlockNumber();
        }

        MulticallBatch batch = braneClient.batch();
        Multicall3 mc3 = batch.bind(Multicall3.class, MULTICALL3_ADDRESS, multicall3Abi);
        BatchHandle<BigInteger> blockHandle = batch.add(mc3.getBlockNumber());

        batch.execute();

        assertTrue(blockHandle.result().success(), "getBlockNumber should succeed");
        assertTrue(blockHandle.result().data().compareTo(BigInteger.ZERO) > 0,
                "Block number should be greater than 0");
    }

    @Test
    void handlesMultipleContractsInSingleBatch() {
        // Deploy a second token with different initial supply
        Address token2Address = deployErc20(BigInteger.valueOf(500_000));

        MulticallBatch batch = braneClient.batch();
        Erc20 token1 = batch.bind(Erc20.class, tokenAddress, ERC20_ABI);
        Erc20 token2 = batch.bind(Erc20.class, token2Address, ERC20_ABI);

        // Mix calls from different contracts
        BatchHandle<BigInteger> balance1 = batch.add(token1.balanceOf(deployer));
        BatchHandle<BigInteger> balance2 = batch.add(token2.balanceOf(deployer));
        BatchHandle<String> symbol1 = batch.add(token1.symbol());
        BatchHandle<String> symbol2 = batch.add(token2.symbol());

        batch.execute();

        // Both tokens should return their respective balances
        assertTrue(balance1.result().success());
        assertTrue(balance2.result().success());
        assertEquals(BigInteger.valueOf(1_000_000), balance1.result().data());
        assertEquals(BigInteger.valueOf(500_000), balance2.result().data());

        // Both tokens have the same symbol (same bytecode)
        assertEquals("BRANE", symbol1.result().data());
        assertEquals("BRANE", symbol2.result().data());
    }

    @Test
    void handlesLargeBatchWithChunking() {
        MulticallBatch batch = braneClient.batch();
        batch.chunkSize(5); // Small chunk size to force multiple RPC calls

        Erc20 token = batch.bind(Erc20.class, tokenAddress, ERC20_ABI);

        // Add 12 calls (will be split into 3 chunks of 5, 5, 2)
        java.util.List<BatchHandle<BigInteger>> handles = new java.util.ArrayList<>();
        for (int i = 0; i < 12; i++) {
            handles.add(batch.add(token.balanceOf(deployer)));
        }

        batch.execute();

        // All calls should succeed with the same balance
        for (int i = 0; i < 12; i++) {
            assertTrue(handles.get(i).result().success(), "Call " + i + " should succeed");
            assertEquals(BigInteger.valueOf(1_000_000), handles.get(i).result().data(),
                    "Call " + i + " should return correct balance");
        }
    }

    @Test
    void handlesCallToNonExistentAddress() {
        // Call balanceOf on an address that has no code (EOA) - should fail
        Address nonExistentContract = new Address("0x0000000000000000000000000000000000000001");

        MulticallBatch batch = braneClient.batch();
        batch.allowFailure(true); // Allow individual failures

        Erc20 token = batch.bind(Erc20.class, nonExistentContract, ERC20_ABI);
        BatchHandle<BigInteger> handle = batch.add(token.balanceOf(deployer));

        batch.execute();

        // The call should fail (no code at that address)
        assertFalse(handle.result().success(), "Call to non-existent contract should fail");
        assertNotNull(handle.result().revertReason());
    }

    private static Address deployErc20(BigInteger initialSupply) {
        Abi abi = Abi.fromJson(ERC20_ABI);
        HexData encodedArgs = abi.encodeConstructor(initialSupply);

        var request = TxBuilder.eip1559()
                .data(new HexData(ERC20_BYTECODE + encodedArgs.value().substring(2)))
                .value(Wei.ZERO)
                .build();

        TransactionReceipt receipt = braneClient.sendTransactionAndWait(request, 10_000, 500);
        assertNotNull(receipt.contractAddress(), "Contract deployment should return address");
        return new Address(receipt.contractAddress().value());
    }
}
