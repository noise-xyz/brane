 ...
 * 2. Call a view method (balanceOf).
 * 3. Call a write method (transfer).
 * 4. Read and decode logs (Transfer event).
 * <p>
 * Usage:
 * ./gradlew :brane-examples:run --no-daemon \
 * -PmainClass=sh.brane.examples.CanonicalErc20Example \
 ...
 * \
 * -Dbrane.examples.contract=0x... (Optional, will deploy if missing)
 */
public final class CanonicalErc20Example {
  ...
  private CanonicalErc20Example() {
    // Prevent instantiation
  }

  // 1. Define the Java interface matching the contract ABI
  ...
  // Record for decoded Transfer event
  public record TransferEvent(Address from, Address to, BigInteger value) {
  }

  public static void main(final String[] args) {
    // 0. Setup configuration from system properties
    ...
    try {
      // 1. Initialize Client
      final var signer = new sh.brane.core.crypto.PrivateKeySigner(privateKey);
      ...
      System.out.println("Contract: " + tokenAddress.value());

      // 2. Bind the Interface
      System.out.println("\n[1] Binding Java Interface...");
      ...
      final BigInteger amount = BigInteger.valueOf(100);
      ...
      // Create a filter for Transfer events in the transaction's block
  ...
  private static String deployErc20(final Brane.Signer client, final BigInteger initialSupply) {
    ...
    System.out.println("DEBUG: Bytecode length: " + ERC20_BYTECODE.length());
    ...
    System.out.println("""
        Usage:
          ./gradlew :brane-examples:run --no-daemon \\
            ...
            -Dbrane.examples.contract=<TOKEN_ADDRESS> (Optional)
        """);
  }
}
