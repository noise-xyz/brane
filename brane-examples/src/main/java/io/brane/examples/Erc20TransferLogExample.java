...
public final class Erc20TransferLogExample {
    ...
    public static void main(String[] args) {
        ...
        final Long fromBlock =
                fromBlockStr != null && !fromBlockStr.isBlank() ? Long.parseLong(fromBlockStr) : null;
        final Long toBlock =
                toBlockStr != null && !toBlockStr.isBlank() && !"latest".equalsIgnoreCase(toBlockStr)
        ...
        for (Transfer t : decoded) {
            // System.out.println("Transfer from " + t.from().value() + " to " + t.to().value() + " amount " + t.value());
