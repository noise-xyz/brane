...
public final class Erc20TransferExample {
  ...
  public static void main(final String[] args) throws Exception {
    ...
    final WalletClient walletClient = DefaultWalletClient.create(provider, publicClient, signer);

    final Abi abi = Abi.fromJson(ERC20_ABI);
    final ReadWriteContract contract = ReadWriteContract.from(token, abi, publicClient, walletClient);

    try {
      final Hash txHash = contract.send("transfer", recipient, amount);
      System.out.println("Sent transfer tx: " + txHash.value());

      final TransactionReceipt receipt = contract.sendAndWait("transfer", 30_000L, 1_000L, recipient, amount);
